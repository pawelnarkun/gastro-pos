...
function renderSelected(selected, config) {
    ...
    if (isPaid) {
        payBlock = `
            <button class="btn btn-success fw-bold w-100 mb-2 py-3" disabled>
                <span class="fs-4">ZAPŁACONE (${selected.payment_method_display || 'OK'})</span>
            </button>
            <div class="text-center text-success fw-bold small">Opłacone — wysłane do kuchni</div>`;
    } else {
        const cardClass = (currentPaymentMethod === 'CARD') ? 'btn-success text-white shadow' : 'btn-outline-success';
        const cashClass = (currentPaymentMethod === 'CASH') ? 'btn-success text-white shadow' : 'btn-outline-success';
        
        payBlock = `
            <div class="mb-2">
                <div class="d-flex gap-2 mb-2 align-items-stretch">
                    
                    <button class="btn ${cardClass} fw-bold py-3" style="flex: 2;" onclick="setPaymentMethod('CARD')">
                        <span class="fs-5">💳 KARTA</span>
                    </button>
                    
                    <button class="btn ${cashClass} fw-bold py-3" style="flex: 2;" onclick="setPaymentMethod('CASH')">
                        <span class="fs-5">💵 GOTÓWKA</span>
                    </button>

                    <button class="btn btn-outline-primary fw-bold p-1" 
                            style="flex: 1; display:flex; flex-direction:column; align-items:center; justify-content:center;" 
                            onclick="openMixedPaymentModal()">
                        <div style="font-size: 1rem;">💳+💵</div>
                        <div style="font-size: 0.9rem;">MIX</div>
                    </button>
                </div>

                <button class="btn btn-primary btn-lg w-100 fw-bold shadow-sm py-3" onclick="markPaid(${selected.id})">
                    ZATWIERDŹ PŁATNOŚĆ (${currentPaymentMethod === 'CARD' ? 'Karta' : 'Gotówka'})
                </button>
            </div>

            <button class="btn btn-outline-dark w-100 mb-2 fw-bold" onclick="openSplitModal()">✂ PODZIEL RACHUNEK (NA OSOBY)</button>
...
// --- PŁATNOŚĆ MIESZANA (MIXED PAYMENT) ---

// Otwiera okno (Zmieniona nazwa funkcji, żeby nie było konfliktu)
function openMixedPaymentModal() {
    if (!currentSelectedData || !currentSelectedData.id) return;
    
    // Używamy zmiennych globalnych zdefiniowanych na górze pliku
    selectedId = currentSelectedData.id;
    currentOrderTotal = parseFloat(currentSelectedData.total_price); 
    
    // Resetuj pola w modalu
    const totalDisplay = document.getElementById('splitTotalDisplay');
    const inputCash = document.getElementById('inputCash');
    const inputCard = document.getElementById('inputCard');
    
    if(totalDisplay) totalDisplay.innerText = currentOrderTotal.toFixed(2);
    if(inputCash) inputCash.value = '';
    if(inputCard) inputCard.value = currentOrderTotal.toFixed(2); // Domyślnie całość kartą
    
    calculateMixedSplit('card'); // Przelicz
...
// Matematyka: Automatycznie uzupełnia drugie pole
function calculateMixedSplit(source) {
    let cashInput = document.getElementById('inputCash');
    let cardInput = document.getElementById('inputCard');
    let btn = document.getElementById('btnConfirmSplit');
    let errorDiv = document.getElementById('splitError');

    let cash = parseFloat(cashInput.value) || 0;
    let card = parseFloat(cardInput.value) || 0;

    // Jeśli wpisuję Gotówkę -> Auto-uzupełnij Kartę
    if (source === 'cash') {
        card = currentOrderTotal - cash;
        // Zaokrąglenie do 2 miejsc po przecinku, żeby uniknąć 10.0000001
        card = Math.round(card * 100) / 100;
        if(card < 0) card = 0;
        cardInput.value = card.toFixed(2);
    } 
    // Jeśli wpisuję Kartę -> Auto-uzupełnij Gotówkę
    else if (source === 'card') {
        cash = currentOrderTotal - card;
    ...
    // Dopuszczamy błąd 1 centa (0.01) przy zaokrągleniach
    if (diff <= 0.01 && cash >= 0 && card >= 0) {
        btn.disabled = false;
        ...
        if(errorDiv) {
            ...
            errorDiv.innerText = "Suma nie pasuje! (" + (sum - currentOrderTotal).toFixed(2) + ")";
        }
    }
}
...
async function submitMixedPayment() {
    ...
    const url = CashierConfig.apiUrl; // np. /kasa/kasa1/api/

    const params = new URLSearchParams();
    params.append('action', 'pay_mixed');
    params.append('order_id', selectedId);
    params.append('cash_amount', cash);
    params.append('card_amount', card);
    ...
    try {
        const resp = await fetch(url, { 
            ...
            headers: {
                ...
                'X-CSRFToken': getCsrfToken() // Używamy poprawnej funkcji tokena
            },
            body: params
        });
        ...
        if (data.status === 'ok') {
            ...
            alert("Błąd: " + (data.message || "Wystąpił błąd płatności"));
        }
    } catch (error) {
